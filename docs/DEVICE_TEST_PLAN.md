...
| ID-10 | P2 | 1.1 | setAppVersion appears as device.version (temp code S3) | S3. | Call `setAppVersion("1.2.3")`, open Home. | Body `device.version: "1.2.3"`. | Used by NPS `appVersionMin/Max` (see NPS-07). | ✅ Run 53: Settings → QA app version `2.5.4` → `App version set to '2.5.4'`, and the next Home bodies carry `device.version: "2.5.4"`; bodies before it had no `version` key. |
...
| TRK-06 | P1 | 1.0 | Empty search query is rejected before the network | S4. | Call `trackSearchView(query: "")`. | Throws `missingRequiredField`; no `Sending POST request` line. | — | ✅ Found and fixed on the branch: the client never called the request's validate() — SearchRequest.validate() rejects an empty query, but push() went straight to the payload, so `trackSearchView(query: "")` would have been sent. push now validates first; RelevaClientValidationTests.testAnEmptySearchQueryIsRejectedBeforeTheNetwork proves the throw (missingRequiredField) with tracking enabled and no network call. |
| TRK-07 | P1 | 1.0 | Search with a filter | S4 variant with `SimpleFilter.priceRange(minPrice: 10, maxPrice: 100)`. | Call trackSearchView with `filter:`. | Body `page.filter` is a JSON object; 200. | 200 proves the filter shape passes the strict schema. | ✅ Run 59: `trackSearchView(query: "shoes", resultProductIds: 3 ids, filter: priceRange 10–200)` → body `page.query`, `page.ids`, `page.filter: {action: include, key: price, operator: "gte,lte", value: "10.0,200.0"}` → 200. |
| TRK-08 | P1 | 1.0 / 4.0 | Full PushRequest builder (temp code S5) | S5. | Push a request with `locale`, `currency`, `pageCategories`, `pageProductIds`, `pageBlocks(tags:)`, `NestedFilter.and(...)`. | All keys present under `page`; 200. | A 400 here names the unknown key in its message; report it as an SDK wire-format bug. | ✅ Run 59 on device: 'Send full PushRequest' → body `page` with url `myapp://consumer.app/home`, locale `bg`, currency `BGN`, ids, categories [Shoes, Sandals], filter → 200. Unit-tested already on the branch: PushRequestTests.testPageBuildersAccumulateUnderPage (locale, currency), testPageProductIdsAndCategoriesAreSetWhenNonEmpty, testPageFilterEmbedsTheEncodedFilter, testAFullyPopulatedRequestIsJsonSerializable — every builder lands under `page`. The harness also has 'Send full PushRequest' in Settings → QA: SDK calls for a 200 on device. |
...
| CART-09 | P2 | 1.0 | Cart totals helpers | S5 scratch. | Build a `Cart` with two products and log `itemCount`, `totalQuantity`, `totalPrice`. | Values correct. | — | ✅ Unit-tested: RelevaClientValidationTests.testCartTotalsHelpers — itemCount counts distinct products (2), totalQuantity sums quantities (3), totalPrice sums price × quantity (25.5), empty cart reports isEmpty and 0. |
...
| TOK-06 | P1 | 1.0 | registerPushToken before setDeviceId (temp code S9) | S9 creates a throwaway client. | Call `registerPushToken("x", deviceType: .ios)` without `setDeviceId`. | Throws `missingRequiredField`; log `ERROR - Cannot register push token without deviceId. Call setDeviceId() first.` | No request. | ✅ Unit-tested: RelevaClientValidationTests.testRegisteringAPushTokenWithoutADeviceIdThrows — a client with no stored device id throws missingRequiredField from registerPushToken before any request; the ERROR log line is emitted with debug logging. |
...
| ID | Prio | Since | Scenario | Precondition | Steps | Expect on device / in log | Expect in backend | Result |
|---|---|---|---|---|---|---|---|---|
| UIK-01 | P1 | 4.2 | BannerPresenter shows a popup | S11 host with `BannerPresenter(host:client:onLinkTap:)`, `start()` in viewWillAppear. Popup fixture returned for the request S11 makes. | Push the scratch controller. | Popup presented `.overFullScreen`; one `Banner impression tracked` line. | One `bannerImpression`. | ✅ Run 56 (harness UIKit host, BannerPresenter over a plain UIViewController): Home screen view → bar aa275c11 and flyout addef82d presented over the host with one `Banner impression tracked` each, the delayed bar f34a090d 3 s later with its own impression; the scroll popup fd4de9a7 appeared as a centred card after 'Report scroll 100 %' (screenshot). Every X produced one `bannerClose` 202. |
| UIK-02 | P1 | 4.2 | Popup and flyout together | Both fixtures. | Push the scratch controller. | Both in one overlay; two impressions; dismissing one keeps the other. | Two impressions. | ✅ Run 56: flyout (edge-flush drawer on the left, full height below the status bar) and popup on screen at the same time in one overlay, portrait and landscape (screenshots). |
| UIK-03 | P1 | 4.2 | Bar in a child controller | Bar fixture. | Push; rotate. | Bar pinned to the safe area edge, correct height; re-lays out on rotation (iOS 16+). | — | ✅ Run 56: bar hosted in the host's own view — pinned to the safe-area top under the navigation bar, full width; after rotating to landscape it re-laid out full width (screenshots). Differs from the SwiftUI path on purpose: no overlay window, so the bar respects the host's safe area instead of running edge to edge. |
...
### Run 53 — NPS cancel complete; app version reaches the body (2026-09-09)

Build with config retention; QA-NPS-01 Custom Event `selectedColor`, delay 8 s, cancel `cartAdd`, Min app 2.0.0; profiles nps-e14, nps-f1.
...
### Run 54 — NPS version gate (2026-09-09)

QA-NPS-08 (Min app 2.0.0, screen view) Active; QA-NPS-01 deactivated during the run; profiles nps-f2, nps-f3; app version set from Settings.
...
### Run 56 — UIKit host: banners over a UIViewController (2026-09-09)

Settings → QA: UIKit surfaces → Open UIKit host; Home page with scroll popup, two bars (immediately / 3 s), flyout; portrait and landscape; alert modal.
...
### Run 58 — rows closed without a device; QA buttons for the rest (2026-09-09)

Unit tests on the branch (RelevaClientValidationTests, PushRequestTests); harness Settings gains 'QA: SDK calls'.
...
### Run 59 — QA SDK calls: merge flow, filters, full request, presets, recommenders (2026-09-09)

Settings → QA: SDK calls; profiles merge-1 → merge-11 → merge2; airplane mode for ID-05; one recommender on Home.
...
### Run 60 — endpoint override through a dev tunnel (2026-09-09)

Settings → API Endpoint Override → devtunnels URL to a local magellan-api; first with a trailing slash, then without.
