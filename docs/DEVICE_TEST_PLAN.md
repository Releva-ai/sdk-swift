...
**Status after 26 run(s)**: 76 pass, 3 fail, 16 pass with caveat, 67 not yet run. Details per row in the Result column and in section 5.
...
| ID | Prio | Since | Scenario | Precondition | Steps | Expect on device / in log | Expect in backend | Result |
|---|---|---|---|---|---|---|---|---|
| BAN-01 | P0 | 1.0.4 | Popup, trigger immediately | Popup banner block on Home. | Open Home. | Popup renders the Unlayer design; log `Banner impression tracked for <blockToken>`; `Sending POST request to …/api/v0/impressions` 200. | Timeline `bannerImpression` with `bannerBlockId = <blockToken>`; banner stats impressions +1 (+2 if the harness double-init fires; note it). | ❌ Runs 1–6: popups rendered as a centred card. Run 21: drawn under the navigation bar → overlay window. Run 23: card stretched to full height → fixed (snapshot). Run 24: popup look confirmed on the device; 'full-bleed' screenshots were the carousel block as a Bar. Run 25 (cold start log): 'BannerOverlay - window created' at launch, popup 4652 queued and impression sent, but no 'showing window' line: Home's onDisappear fired during launch and detached the screen from the window, so the popup only appeared after a tab switch. Fixed: a started screen that receives an overlay banner re-attaches itself. Impression is still sent when the banner is queued, not when it is on screen — an over-count in this scenario; decide with the CTO whether to move it to first display. Re-test a cold start. |
