...
**Status after 37 run(s)**: 82 pass, 1 fail, 18 pass with caveat, 61 not yet run. Details per row in the Result column and in section 5.
...
| BAN-15 | P1 | 1.2 | sessionInterval | Block with sessionInterval 2. | Cold start twice. | Shown only when `device.sessions % 2 == 0`. | — | ⚠️ Run 36: cold starts sessions 73→78 with a sessionInterval block (7612e7c4-…) returned on some launches; it was displayed when returned, but the interval could not be read from the log and the normal popup was overwritten by it (BAN-18 bug). Re-test after rebuild: note the block's interval and which session numbers return it (device.sessions in the request body). |
...
| BAN-17 | P1 | 1.0.4 | Banner without a design is ignored | Block with html only (design null) or displayType custom. | Open Home. | Nothing rendered, no impression, no crash. | No event. | ⚠️ Run 36: a popup with an empty Unlayer design (f3616491-…) was shown as an empty card and counted. The SDK skipped only a missing design. Fixed: a design with no rows or no content is skipped and not counted (test added). Re-test with the same block; also test a block with design null if the admin allows it. |
| BAN-18 | P1 | 1.0.4 | Two banners at once | Popup + bar both immediate. | Open Home. | Both render; closing one leaves the other. | Two impressions. | ⚠️ Run 14: two banners (popup + carousel popup) showed one after the other. Run 36 with two immediate popups on Home: both impressions were sent at once but only the second was displayed — the first was replaced before anyone saw it (also the 'session banner skipped the normal one' observation). Fixed: popups and flyouts queue; the next shows, and is counted, when the current is closed (test added). Re-test: two immediate popups → first shows, close → second shows, two impressions in that order. |
