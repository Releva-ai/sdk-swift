...
**Status after 24 run(s)**: 76 pass, 2 fail, 17 pass with caveat, 67 not yet run. Details per row in the Result column and in section 5.
...
| ID | Prio | Since | Scenario | Precondition | Steps | Expect on device / in log | Expect in backend | Result |
|---|---|---|---|---|---|---|---|---|
| BAN-01 | P0 | 1.0.4 | Popup, trigger immediately | Popup banner block on Home. | Open Home. | Popup renders the Unlayer design; log `Banner impression tracked for <blockToken>`; `Sending POST request to …/api/v0/impressions` 200. | Timeline `bannerImpression` with `bannerBlockId = <blockToken>`; banner stats impressions +1 (+2 if the harness double-init fires; note it). | ⚠️ Runs 1–6: popups render with one impression per display; run 6: centred card, X inside, overlay dim from the design. Run 21: drawn under the navigation bar → now hosted in the overlay window. Run 23 (simulator snapshot): the card always stretched to the full available height with blank card colour above and below a short design, because a frame(maxHeight:) modifier grows to whatever height is offered; in the window that is the whole screen, so the popup looked full-screen. Fixed: the card now takes its content's height, capped to the safe area with scrolling beyond it; snapshot shows the reference look (card with margins, radius, X in the corner, dim). Re-test on the device. |
