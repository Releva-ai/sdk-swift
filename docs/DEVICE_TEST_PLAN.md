...
**Status after 25 run(s)**: 76 pass, 2 fail, 17 pass with caveat, 67 not yet run. Details per row in the Result column and in section 5.
...
| ID | Prio | Since | Scenario | Precondition | Steps | Expect on device / in log | Expect in backend | Result |
|---|---|---|---|---|---|---|---|---|
| BAN-01 | P0 | 1.0.4 | Popup, trigger immediately | Popup banner block on Home. | Open Home. | Popup renders the Unlayer design; log `Banner impression tracked for <blockToken>`; `Sending POST request to …/api/v0/impressions` 200. | Timeline `bannerImpression` with `bannerBlockId = <blockToken>`; banner stats impressions +1 (+2 if the harness double-init fires; note it). | ⚠️ Runs 1–6: popups rendered as a centred card (reference look 2026-09-03 22:51). Run 21: drawn under the navigation bar → moved to the overlay window. Run 23: card stretched to full height → fixed (snapshot). Run 24: the 'full-bleed popup' screenshots turned out to be the carousel block configured as a Bar; the tester confirmed on the device that the popup itself looks right (margins, radius, dim, X). Still open from run 24: on a cold start the popup impression was tracked with nothing drawn (overlay window not yet created) — fixed in 4253594, re-test a cold start. |
