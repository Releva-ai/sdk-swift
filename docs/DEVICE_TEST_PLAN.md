...
**Status after 25 run(s)**: 76 pass, 3 fail, 16 pass with caveat, 67 not yet run. Details per row in the Result column and in section 5.
...
| ID | Prio | Since | Scenario | Precondition | Steps | Expect on device / in log | Expect in backend | Result |
|---|---|---|---|---|---|---|---|---|
| BAN-01 | P0 | 1.0.4 | Popup, trigger immediately | Popup banner block on Home. | Open Home. | Popup renders the Unlayer design; log `Banner impression tracked for <blockToken>`; `Sending POST request to …/api/v0/impressions` 200. | Timeline `bannerImpression` with `bannerBlockId = <blockToken>`; banner stats impressions +1 (+2 if the harness double-init fires; note it). | ❌ Runs 1–6: popups rendered as a centred card (reference look 2026-09-03 22:51). Run 21: drawn under the navigation bar → moved to the overlay window. Run 23: card stretched to full height → fixed (snapshot). Run 24 (device, tester): two problems remain since the window move — (1) on a cold start or debug launch the popup's impression is tracked but nothing is drawn: the overlay window is created only when the Home screen appears, and at that moment no window scene may be connected yet, so a banner that arrives right after has nowhere to draw; the window is now also created on scene activation and on the first banner, and the SDK logs 'BannerOverlay - …' lines (window created / deferred / showing / hiding) so the next log shows the path; (2) the popup on the device still shows without the side margins the reference has. Needs a screenshot of the current build plus the BannerOverlay log lines. Impression-without-display is a real over-count while it lasts. |
...
| BAN-05 | P0 | 1.0.4 | Flyout left and right | Two flyout blocks. | Open Home. | Side panels slide in from the configured side; close works. | Impressions and closes. | ☐ |
...
- Popup margins still not as the reference on the device; the simulator snapshot shows margins, so the difference is device-side and needs the current screenshot + log to pin down.
