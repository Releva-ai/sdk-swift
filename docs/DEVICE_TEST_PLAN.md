...
**Status after 34 run(s)**: 80 pass, 1 fail, 17 pass with caveat, 64 not yet run. Details per row in the Result column and in section 5.
...
| BAN-08 | P0 | 1.0.4 | scrollPercentage 50 | Trigger scrollPercentage 50. | Open Home, scroll halfway. | Appears when the grid passes 50 %. | Impression. | ⚠️ Run 33: scroll 20 % and 50 % never fired. SDK cause: the banner and story managers were always initialised with a nil scroll provider, so the trigger was dead on iOS. Fixed on the branch: RelevaClient.reportScrollPercentage(0…100) drives BannerManagerService.onScroll / StoryManagerService.onScroll; the harness reports Home and product-page scroll progress. README: apps must call reportScrollPercentage from their scroll views. Re-test after rebuild. |
| BAN-09 | P0 | 1.0.4 | cartChanged and wishlistChanged triggers | Two blocks with those triggers. | Open Home → Product 1 → Add to cart → back to Home. Then heart a product on Home. | Banner appears after the cart/wishlist change (`RelevaClient.setCart` calls the SDK's own `BannerManagerService.onCartChanged`). | Impression. | ⚠️ Run 33: setCart already calls bannerManager.onCartChanged, but the product page had no page token and no banner surface, so a cartChanged banner returned for it had nowhere to show and the harness's duplicate BannerManagerService re-ran triggers. Harness now sends the product page token 6b9841dd-… on product views and the cart page token 1962cd17-… on the cart screen, attaches banner display to both screens, and no longer runs its own manager. Re-test: add to cart on the product page with a cartChanged block on the product page; heart a product for wishlistChanged. |
