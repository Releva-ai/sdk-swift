...
- **`BannerPresenterTests` and `NpsPresenterTests`**, driving each presenter through the display-controller singleton its production publisher comes from, with a `BannerTracker` spy in place of `RelevaClient` so no test performs network I/O. They cover: a popup reaching the screen as an `.overFullScreen` hosting controller with its impression counted; a popup and a flyout arriving together sharing one presented controller and counting two impressions; a bar banner going into a child view controller rather than a modal; a static banner being neither shown nor counted; dismissing the popup taking the overlay down and reporting `bannerClose`; `stop()` dismissing, removing the bar slots and ignoring later banners; and for NPS, a survey presented as a `.pageSheet`, a second survey ignored while one is on screen, and `stop()` dismissing and unsubscribing. The shared `PresentationTestSupport` helpers wait two ways for two different reasons: a marker queued behind the `receive(on: DispatchQueue.main)` hops makes the "must not happen" assertions exact rather than a race against an inverted expectation, while the presentation and dismissal assertions pump the run loop until the condition holds, because an animation cannot be queued behind.
...
- **`BannerChrome` lost its main-actor isolation in the move out of `BannerDisplayModifier`.** The modifier is a `ViewModifier`, which is `@MainActor` by protocol, so the chrome's closures were isolated for free while they lived inside it; a bare `enum` gets no such inference, and those closures call main-actor-isolated `BannerDisplayViewModel` methods (`dismissPopup`, `dismissFlyout`, `dismissBar`, `trackClick`) — an actor-isolation error under Swift 5 language mode, and the reason CI failed on this PR. `BannerChrome` is now `@MainActor` itself.
- **`BannerPresenter`'s overlay-dismissal flag could latch on permanently.** `isDismissingOverlay` was set unconditionally but only cleared inside `dismiss`'s completion, which optional chaining skipped whenever `presentingViewController` was already `nil` — reachable if the presentation never took (`topMostPresentedViewController` can hand back a controller whose own dismissal is still in flight, so `present` logs "already presenting" and does nothing) or if something else had already taken the overlay down. Once latched, the presenter silently stopped showing popups and flyouts until a `stop()` / `start()` cycle. `syncOverlay` now checks for a controller that was never actually presented before deciding there is nothing to dismiss, on both the presenting and the dismissing path, so a failed or already-gone presentation is dropped and retried instead of wedging the flag.
- **The iOS 15 bar-measurement fallback could measure against a zero width.** `measureBars()` could run, from `start()`'s first reconcile, before the host's first layout pass — most reachable after a `stop()` / `start()` round trip, which leaves `barBanners` already populated. It now forces layout first and skips a measurement against a zero width rather than sizing the bar to it, and it no longer forces that layout pass at all on iOS 16 and up, where no slot has a height constraint to size in the first place.
- **A presentation that silently failed to take could strand the pending banner or survey.** `present`/`syncOverlay` only run again when a new banner or survey event arrives, so a call that UIKit logged rather than honoured (for example landing on a host mid-transition) left the current one drawn nowhere until an unrelated event happened to trigger the next reconcile. Both presenters now check `presentingViewController` right after presenting — UIKit sets it synchronously when a presentation takes — and retry up to twice on the next run-loop turn before giving up.

### Known limitations

- `BannerPresenter.stop()` leaves the view model's banner state as it was, so a later `start()` re-presents whatever was on screen without a second impression — the same behaviour the SwiftUI modifier's `@StateObject` already has across `onDisappear`, now stated on `stop()`'s doc comment. There is no `deinit` fallback: a presenter released while started (a host deallocated without a matching `viewWillDisappear`, or an app that forgets `stop()`) leaves any presented overlay on screen with nothing left to dismiss it — call `stop()` to avoid that.
...
- The bar-measurement fix above addresses the zero-width case; a second hazard the review raised — whether `sizeThatFits(in:)` sees a bar's new content or the previous turn's, since it runs in the same run-loop turn the state change was observed — needs a simulator to observe and is not verified here, consistent with this PR originating on Linux with no Swift toolchain.
