...
- **Code coverage in CI.** The test step now passes `-enableCodeCoverage YES -resultBundlePath TestResults.xcresult`, and a new step prints per-target and per-file line coverage from `xcrun xccov view --report --json` and fails the build if coverage for the shipped targets (test targets excluded) drops below the `MIN_LINE_COVERAGE` floor set in the workflow.
