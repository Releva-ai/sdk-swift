...
- **The iOS 15 bar-measurement fallback could measure against a zero width.** `measureBars()` could run, from `start()`'s first reconcile, before the host's first layout pass — most reachable after a `stop()` / `start()` round trip, which leaves `barBanners` already populated. It now forces layout first and skips a measurement against a zero width rather than sizing the bar to it.

### Known limitations

- `BannerPresenter.stop()` leaves the view model's banner state as it was, so a later `start()` re-presents whatever was on screen without a second impression — the same behaviour the SwiftUI modifier's `@StateObject` already has across `onDisappear`, now stated on `stop()`'s doc comment and covered by `testANewPopupAfterDismissalIsPresentedAgain`'s sibling in intent. There is no `deinit` fallback: a presenter released while started (a host deallocated without a matching `viewWillDisappear`, or an app that forgets `stop()`) leaves any presented overlay on screen with nothing left to dismiss it. A synchronous `deinit` cannot itself be main-actor-isolated the way the rest of this class is, so a correct fix needs either isolated-deinit support or an async teardown path — left to a follow-up rather than risking a second isolation mistake in the same PR that just had one caught by CI.
