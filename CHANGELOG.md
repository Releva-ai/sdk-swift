...
- **`RecommenderResponse.meta` and `ProductRecommendation.custom` / `.data` were unconditionally discarded.** Their `init(from:)` assigned `nil` with a comment saying `[String: Any]` cannot be decoded by `Codable`, and `encode(to:)` skipped them with a matching note, so a caller reading a recommender's `meta` or a product's `custom` fields always got `nil` no matter what the API sent. They now decode and re-encode as `JSONValue`.

### Added

- `JSONValueTests`, covering the number fidelity that motivated the separate `int` case (a decode→encode round-trip is byte-compared against the original text), the accessors returning `nil` for a mismatched case rather than coercing, the subscripts returning `nil` rather than trapping on a type or index mismatch, and the `JSONSerialization` bridge in both directions — including that a JSON boolean does not come back as the number 1 and that `anyValue` output still passes `isValidJSONObject`, which is what `InboxService` relies on when it persists inbox state.
...
- `RecommenderResponseTests` coverage that `meta` / `custom` / `data` survive a decode and a re-encode, replacing the three tests that pinned the old drop-on-decode behaviour.
